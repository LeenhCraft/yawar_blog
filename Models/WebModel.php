...
class WebModel extends Mysql
{
    ...
    {
        $sql = "SELECT * FROM sis_articulos WHERE art_estado = 1 ORDER BY idarticulo DESC LIMIT $cant";
        $request = $this->select_all($sql);
        if (!empty($request)) {
    ...
    {
        $sql = "SELECT * FROM sis_articulos WHERE idarticulo = $id AND art_estado = 1";
        $request = $this->select($sql);
        return $request;
    }
    ...
    {
        $sql = "SELECT * FROM sis_articulos WHERE idarticulo = $id AND art_estado = 1 AND art_stock >= $cant";
        $request = $this->select($sql);
        return $request;
    }
    ...
    {
        $sql = "SELECT SUM(a.car_cantidad) as car_cantidad FROM web_carritos a 
        INNER JOIN web_visitas b ON b.vis_cod=a.vis_cod
        WHERE b.vis_cod = '$idvisita' AND a.car_anulado = 0 AND a.idpedido = 0";
        $request = $this->select($sql);
    ...
    {
        $sql = "SELECT * FROM web_carritos WHERE idarticulo = $id AND idvisita = $idvisita AND car_anulado = 0 AND idpedido = 0";
        $request = $this->select($sql);
        return $request;
    }
    ...
    {
        $vis_cod = $_SESSION['vi'];
        $sql = "SELECT * FROM web_carritos WHERE vis_cod='$vis_cod' AND idpedido = 0";
        $request = $this->select_all($sql);
        if (!empty($request)) {
            $sql = "UPDATE web_carritos SET idwebusuario = ? WHERE vis_cod = ? AND idpedido = 0";
    ...
    {
        $sql = "SELECT * FROM web_banners WHERE ban_estado = 1";
        $request = $this->select_all($sql);
        return $request;
    }
    ...
    {
        $sql = "SELECT * FROM web_publicidades WHERE pub_estado = 1";
        $request = $this->select_all($sql);
        return $request;
    }
    ...
    {
        $sql = "SELECT * FROM web_masvisitados WHERE vis_cod = $vis_cod AND idarticulo = $idarticulo";
        $response = $this->select_all($sql);
        if (empty($response)) {
            $sql = "INSERT INTO web_masvisitados(vis_cod,idarticulo,mas_url) VALUES (?,?,?)";
